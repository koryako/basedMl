...
import glob
import os
import util.ganbaselineModel as model
import scipy
...
def train(path, batch_size, EPOCHS):
    ...
    discriminator = model.discriminator_model()#创建对抗起
    generator = model.generator_model()#创建生成器
    ...
    adam_gen=Adam(lr=0.00002, beta_1=0.0005, beta_2=0.999, epsilon=1e-08)#创建生成优化器
    adam_dis=Adam(lr=0.00002, beta_1=0.0005, beta_2=0.999, epsilon=1e-08)#创建对抗优化器
    #opt = RMSprop()
